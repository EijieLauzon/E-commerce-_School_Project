...
function TryComponents({ isLoading, setIsLoading, setFetchedOTP, setError, setDebugOTP }) {
   const router = useRouter()
   const pathname = usePathname()
   const searchParams = useSearchParams()
   const method = searchParams.get('method')
...
function VerifyComponents({ isLoading, setIsLoading, setError }) {
   const router = useRouter()
   const pathname = usePathname()
   const searchParams = useSearchParams()
   const method = searchParams.get('method')
   ...
   async function onVerifyOTP() {
      try {
         ...
         if (response.ok) {
            ...
            const data = await response.json().catch(() => null)
            setError(data?.message ?? 'Could not verify the login code.')
         }
      } catch (error) {
         console.error({ error })
         setError('Could not verify the login code.')
      } finally {
         setIsLoading(false)
      }
   }
